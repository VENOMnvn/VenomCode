...
const getConversations = async (req,res)=>{
    try{
        ...
        console.log(id);
        const conversations = await Conversation.find({users:id}).populate('users','profilePicture -_id firstname lastname username');
...
const getChat = async (req,res)=>{
    try{
        ...
        if(username){
            ...
            const conv = await Conversation.findOne({$and:[{users:userid},{users:userresponse._id}]} );
            ...
            if(conv){
              ...
              let conv = await Conversation.findById(id).populate({path:"messages"}).populate({path:"users"});
              conv.users.map((user)=>{
                user.password = null
                user._id=null
                user.dob=null
...
const sendMessage = async (req,res)=>{
    try{
        ...
        const response = await Conversation.findByIdAndUpdate(req.body?.chatID,{$push:{messages:msgCreate._id}})
